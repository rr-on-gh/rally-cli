...
var hr = _.repeat('_', process.stdout.columns);
...
var printTasks = function(tasks, project, callback) {
    var available = process.stdout.columns - header.length - ' US99999 '.length;
    for (var i in tasks) {
        var aResult = tasks[i];
        // console.log(aResult);
        console.log('%s | %s | %s | %s | %s | %s | %s | %s | %s', aResult.FormattedID, trimTo(aResult.Estimate, 3),
            trimTo(aResult.ToDo, 4), trimTo(aResult.Actuals, 3), colorizeOnState(trimTo(aResult.State, 12),
                aResult.State), trimTo(aResult.Name, available, 50), aResult.WorkProduct.FormattedID, trimTo(aResult.WorkProduct.Name, available, 30),
            trimTo(project.name, available, 10));
...
var completion = function(estimate, todo, s, e) {
    ...
    c.remainingPctFormatted = completion >= remainingPct ? '\x1b[32m' + completion + '%\x1b[0m' : '\x1b[33m' + completion + '%\x1b[0m';
    return c;
};
